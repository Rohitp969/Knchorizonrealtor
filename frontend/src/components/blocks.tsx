...
export function ContactForm({ compact = false, propertySlug, projectSlug, inquiryType = 'contact' }: { compact?: boolean; propertySlug?: string; projectSlug?: string; inquiryType?: string }) {
  ...
  return (
    <form onSubmit={submit} className={`grid gap-5 ${compact ? '' : 'md:grid-cols-2 md:gap-x-7'}`} data-testid="form-contact">
      ...
      <label className="block relative"><span className="eyebrow text-[#202635]/45">I’m looking to</span><select required value={form.interest} onChange={update('interest')} className="mt-3 w-full appearance-none border-b border-[#202635]/25 bg-transparent py-3 text-base outline-none focus:border-[#c97352]" data-testid="select-contact-interest"><option value="">Select an option</option><option>Buy a property</option><option>Sell a property</option><option>Rent a home</option><option>Explore an investment</option></select><ChevronDown size={15} className="pointer-events-none absolute bottom-3 right-1 text-[#202635]/50" /></label>
      <label className="block md:col-span-2"><span className="eyebrow text-[#202635]/45">A little about your plans</span><textarea required value={form.message} onChange={update('message')} rows={3} className="mt-3 w-full resize-none border-b border-[#202635]/25 bg-transparent py-3 text-base outline-none transition-colors placeholder:text-[#202635]/30 focus:border-[#c97352]" placeholder="Tell us what would make this move feel right." data-testid="textarea-contact-message" /></label>
      {error && <p className="text-sm text-[#c97352] md:col-span-2" role="alert">{error}</p>}
      <button disabled={submitting} type="submit" className="group mt-3 flex w-fit items-center gap-3 bg-[#202635] px-6 py-4 font-mono text-[10px] uppercase tracking-[.15em] text-[#f5f0e6] transition-colors hover:bg-[#c97352] disabled:cursor-wait disabled:opacity-60 md:col-span-2" data-testid="button-contact-submit">{submitting ? 'Sending…' : 'Send enquiry'} <Send size={14} className="transition-transform group-hover:translate-x-1" /></button>
    </form>
  );
}
